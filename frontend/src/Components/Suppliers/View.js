...
export function View(props) {
    ...
    useEffect(() => { axios.get('/suppliers').then(sup => { setSuppliers(sup.data) }) }, [])

    function DisplaySuppliers(searchText) {
        let supplierList = suppliers
        if (searchText) {
            supplierList = supplierList.filter(function (supplier) {
                return supplier.first_name.toLowerCase().includes(searchText.toLowerCase()) ||
    ...
    return (
        <table className="table table-striped">
            ...
            <tbody>
                {DisplaySuppliers(props.searchText)}
            </tbody>
        </table>
    );
}